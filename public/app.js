...
const channel = typeof BroadcastChannel !== 'undefined' ? new BroadcastChannel('bushrakom-session') : null;
...
async function api(path, { method = 'GET', data } = {}) {
  const res = await fetch(path, { method, headers: data === undefined ? {} : { 'Content-Type': 'application/json' }, body: data === undefined ? undefined : JSON.stringify(data), credentials: 'same-origin', cache: 'no-store' });
  const result = await res.json();
  if (!res.ok) {
    if (res.status === 401 && path !== '/api/login') showLogin(result.error);
    throw new Error(result.error || 'تعذّر إكمال العملية.');
  }
  return result;
}
...
async function heartbeat() {
  ...
  catch (e) { if (currentUser) { activityPending = true; notice('تعذّر تأكيد اتصال الجلسة. تحقق من الاتصال.', true); } }
...
function updateUpload() {
  ...
  $('#file-hint').textContent = selected ? 'جميع أنواع الملفات مقبولة، حتى ١ جيجابايت لكل ملف.' : 'حدد المجلد لتتمكن من اختيار الملف.';
}
...
function renderFiles() {
  ...
  for (const f of visible) {
    ...
    const download = node('a','تنزيل','download'); download.href = `/api/files/${f.id}/download`; download.download = f.name; download.setAttribute('aria-label',`تنزيل ${f.name}`); actions.append(download);
...
$('#upload-form').addEventListener('submit',event=>{
  ...
  const xhr = new XMLHttpRequest(); activeUpload = xhr; updateUpload();
  $('#upload-progress-wrap').hidden = false; $('#upload-progress').value = 0; $('#upload-status').textContent = 'جارٍ رفع الملف…';
  xhr.open('POST',`/api/files?folderId=${encodeURIComponent(folder)}`); xhr.setRequestHeader('X-File-Name',encodeURIComponent(file.name)); xhr.setRequestHeader('Content-Type','application/octet-stream');
  xhr.upload.onprogress = event => { if (event.lengthComputable) { const percent = Math.round(event.loaded/event.total*100); $('#upload-progress').value = percent; $('#upload-status').textContent = percent === 100 ? 'اكتمل الإرسال، جارٍ حفظ الملف…' : `جارٍ رفع الملف… ${formatNumber(percent)}٪`; } };
  xhr.onload = async () => {
    let data; try { data = JSON.parse(xhr.responseText); } catch { return notice('تعذّر قراءة استجابة الخادم.',true); }
    if (xhr.status === 401) return showLogin(data.error);
    if (xhr.status >= 400) return notice(data.error || 'تعذّر رفع الملف.',true);
    $('#upload-file').value = ''; notice(`تم رفع «${file.name}» بنجاح.`); try { await loadFiles(); } catch(e) { showError(e); }
  };
  xhr.onerror = ()=>{if(currentUser)notice('توقف الرفع بسبب الاتصال أو رفض الخادم للملف. حاول مجددًا.',true);};
  xhr.onabort = ()=>{if(currentUser)notice('تم إلغاء رفع الملف.');};
  xhr.onloadend = ()=>{activeUpload=null;$('#upload-progress-wrap').hidden=true;updateUpload();};
  xhr.send(file);
});
...
// The server remains authoritative; merely opening or polling a page never extends a session.
(async()=>{try{await enterPortal(await api('/api/me'));}catch(e){if(currentUser)showError(e);else showLogin();}})();
